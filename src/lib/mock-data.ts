...
export const initialEvents: CalendarEvent[] = [
    { id: 1, date: new Date(), title: 'Audição de Piano', description: 'Audições semestrais para alunos de piano.', type: 'Audição', time: '14:00 - 17:00', location: 'Auditório Principal', status: 'Próxima', participants: initialStudents.filter(s => s.instrument === 'Piano').slice(0,4).map(s => ({id: s.id!, name: s.name})) },
    ...
    { id: 7, date: new Date(new Date().setDate(new Date().getDate() + 40)), title: 'Audição de Sopros', description: 'Audição para alunos de flauta e clarinete.', type: 'Audição', time: '16:00', location: 'Auditório Principal', status: 'Próxima', participants: initialStudents.filter(s => s.instrument === 'Flauta').map(s => ({id: s.id!, name: s.name})) }
